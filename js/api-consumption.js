...
document.querySelectorAll("button").forEach(menuOption => {
    menuOption.addEventListener("click", function() {
        document.querySelector(".selected").removeAttribute("class", "selected");
...
function apiFetch (choosenEndPoint){fetch(`${api}${choosenEndPoint}?language=en-US&page=1`, options)
  ...
  .then((data) => {
    ...
    data.results.forEach(movie => {
        ...
        newMovie.querySelector("img").setAttribute("src", `https://image.tmdb.org/t/p/w500${movie.poster_path}`);
